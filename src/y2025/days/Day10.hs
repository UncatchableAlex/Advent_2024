...
import UTIL.LA (gausselim, matvec)
  ...
  where
    ...
    fixMachine (_, buttons, dials) =
      let n = length dials
          ...
          (ref, frees) = gausselim arr
          x = backSub ref frees arrInt dials
       in sum x

-- Branch and prune a solution on the free variables using back substitution.
...
backSub ref frees buttons dials = A.elems $ fix xInit m n bestInit
  where
    ...
    fix xs i j best
      ...
      | j `elem` frees =
          let -- where the dials are currently at with our partial solution
           ...
           in -- test every possible value for xj, accumulating the best solution we have found up
      ...
      | otherwise =
          let s = sum [(toInteger (xs ! k) % 1) * ref ! (i, k) | k <- [j + 1 .. n]]
              xj = (ref ! (i, n') - s) / ref ! (i, j)
              xjInt = fromInteger $ numerator xj
              xs' = xs // [(j, xjInt)]
           in if denominator xj == 1 && xj >= 0
