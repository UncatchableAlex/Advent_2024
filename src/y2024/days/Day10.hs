module Y2024.DAYS.Day10 (day10) where
  ...
  where
    ...
    bfs (x', y') (x, y) -- our parameters are the previous position and the current position
      | x < xmin
          ...
          || y < ymin
          || y > ymax
          || ((x, y) /= (x', y') && topoMap ! (x, y) - topoMap ! (x', y') /= 1) -- if this position is invalid return the empty map
        =
          M.empty
...
day10 :: IO (Int, Int)
day10 = do
  input <- readFile "src/y2024/inputs/day10.txt"
