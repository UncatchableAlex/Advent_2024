module Y2024.DAYS.Day18 (day18) where
  ...
  PQ.pop q >>= \((s, (x, y)), q') ->
    if (x, y) == end
      then Just s
        ...
        let candidates = zip (repeat (s + 1)) [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]
            validMove (a, b) =
              a >= 0
                && b >= 0
                && a <= height
                && b <= width
                && S.notMember (a, b) visited
                && (ram A.! (a, b))
...
day18 :: IO (Int, Int)
day18 = do
  input <- readFile "src/y2024/inputs/day18.txt"
