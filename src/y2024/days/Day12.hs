module Y2024.DAYS.Day12 (day12) where
...
compute plots = (p1, p2)
  where
    ...
    step (p1', p2', unvisitedPlots) =
      let searchStart = (M.keys unvisitedPlots) !! 0
    ...
    bfs visited color (x, y)
      ...
      | otherwise =
          let neighbors = [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)]
           in foldl' (foldFunction color) (S.insert (x, y) visited, 0) neighbors

-- expect (1518548,909564)
day12 :: IO Pos
day12 = do
  input <- readFile "src/y2024/inputs/day12.txt"
  let plots =
        M.fromList $
          [ ((i, j), x)
